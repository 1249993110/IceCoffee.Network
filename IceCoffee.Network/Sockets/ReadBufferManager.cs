...
namespace IceCoffee.Network.Sockets
{
    public class ReadBufferManager
    {
        ...
        private Queue<SocketAsyncEventArgs> readBuffer;
        ...
        /// 回收RecvSaea
        /// </summary>
        private Action<SocketAsyncEventArgs> _collectSaea;
        ...
        {
            get { return _readBufferMaxLength * _session.SocketDispatcher.ReceiveBufferSize; }
        ...
        {
            _newlineIndex = -1;
            if (readBuffer.Count == 0 || maxSize <= 0)//缓冲区为空
            {
                return new byte[0];
            }
            ...
            else//移除部分,此时队列中应至少有1个saea
            {
                ...
                SocketAsyncEventArgs saea = readBuffer.Peek();
        ...
        public byte[] ReadAll()
        {
            ...
            var firstItem = readBuffer.Dequeue();
        ...
        /// <summary>
        ///  搜索指定的byte对象，并返回读取缓存中第一个匹配项的索引，索引基于_readOffset偏移。
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        public long IndexOf(byte value)
        {
            if (readBuffer.Count == 0)//未找到
            {
                return -1L;
            }
        ...
        {
            readBuffer = new Queue<SocketAsyncEventArgs>();
            this._onInternalReceived = onInternalReceived;
        }

        internal void Initialize(Action<SocketAsyncEventArgs> saeaCollectEventHandler, ISession session)
        {
        ...
        {
            if(_readBufferMaxLength <= 0 || readBuffer.Count < _readBufferMaxLength)
            {
                readBuffer.Enqueue(saea);
                _bytesAvailable += saea.BytesTransferred;
